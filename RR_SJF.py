...
def preemptive_sjf_scheduling(total_time, processes):
    ...
    for p in completed_processes:
        print(f"{p.name} wait\t{p.wait_time} turnaround\t{p.turnaround_time} response\t{p.response_time}")
...
def round_robin(processes, quantum, arrival_arr, burst_arr,process_name_arr):
    ...
    while any(remaining_time.values()) and unfinished_jobs:
        if not ready_queue and unfinished_jobs:
            ...
            print(f"Time  {current_time:3d} : {ready_queue[0]['job']} selected (burst {ready_queue[0]['bt']:3d})")
        ...
        if remaining_time[process_to_execute["job"]] <= time_quantum:
            ...
            print(f"Time  {current_time:3d} : {process_to_execute['job']} finished")
            ...
            print(f"Time  {current_time:3d} : {process_to_execute['job']} selected (burst {remaining_time[process_to_execute['job']]:3d})")
    ...
    total_rt = sum(process["tat"] - process["bt"] for process in solved_processes_info)
    ...
    for process in solved_processes_info:
        print(f"{process['job']}   wait {process['wat']:3d} turnaround {process['tat']:3d} response {process['rt']:3d}")
